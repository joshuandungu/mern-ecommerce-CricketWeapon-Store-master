...
exports.newOrder = asyncWrapper(async (req, res, next) => {
  ...
  const needsFuturePayment = paymentInfo.status === "Cash on Delivery" || paymentInfo.status === "M-Pesa Processing";
  const paidAt = needsFuturePayment ? null : Date.now();
  const orderStatus = needsFuturePayment ? "Processing" : "Paid";
